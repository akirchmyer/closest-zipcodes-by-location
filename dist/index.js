...
var ZipCodes = function () {
	...
	_createClass(ZipCodes, [{
		...
		key: 'set',
		value: function set(zips) {
			if (zips && zips.length) {
				this.locations = this.defaultLocations.filter(function (loc) {
		...
		key: 'closest',
		value: function closest(zip, count) {
			var selected = this.locations.find(function (loc) {
				return loc.zip === zip;
			});
			return _geolib2.default.orderByDistance(selected, this.locations).slice(0, count);
		}
	}]);
